...
//  CREDENTIAL TESTS
	...
	public void editCredTest() throws InterruptedException {
		...
		homePage.selectCredentialsWindow();
		Thread.sleep(500);
		Credential beforeCred = new Credential(null, "adiii", "ageel", null, "abc");
		Credential newCred = new Credential(null, "adiii", "alageel", null, "def");
		homePage.addCred(beforeCred.getUrl(), beforeCred.getUsername(), beforeCred.getPassword());
		homePage.logout();
		Thread.sleep(500);
		driver.get("http://localhost:" + this.port + "/login");
		loginPage = new LoginPage(driver);
	...
	public void addNoteTest() throws InterruptedException {
		...
		Note note = new Note(null, "Hello", "How are you");
